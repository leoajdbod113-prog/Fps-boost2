...
CopyButton.Size = UDim2.new(0, 110, 0, 36)
CopyButton.Position = UDim2.new(0, 250, 0, 75)
CopyButton.BackgroundColor3 = Color3.fromRGB(45, 80, 120)
...
local function buildClipboardText()
    local lines = {}
    table.insert(lines, "=== NEXUS BLOCK-ON INSPECTOR ===")
    table.insert(lines, string.format("GUI: %d | CANDIDATOS: %d | API: %d", #State.GuiResults, #State.GuiCandidates, #State.ApiResults))
    table.insert(lines, "")
    table.insert(lines, "=== GUI BUTTON CANDIDATES ===")
    table.sort(State.GuiCandidates, function(a, b) return a.Score > b.Score end)
    for i, data in ipairs(State.GuiCandidates) do
        if i > MAX_CANDIDATES then break end
        table.insert(lines, string.format("[CANDIDATE %d] Name=%s | Class=%s | Visible=%s | Active=%s | Score=%d | Path=%s", i, data.Name, data.ClassName, tostring(data.Visible), tostring(data.Active), data.Score, data.Path))
        if data.Image then table.insert(lines, "Image=" .. tostring(data.Image)) end
        if data.Text then table.insert(lines, "Text=" .. tostring(data.Text)) end
    end
    table.insert(lines, "")
    table.insert(lines, "=== ALL GUI BUTTONS ===")
    for i, data in ipairs(State.GuiResults) do
        table.insert(lines, string.format("[GUI %d] %s | %s | %s", i, data.Name, data.ClassName, data.Path))
    end
    table.insert(lines, "")
    table.insert(lines, "=== API / OBJECT CANDIDATES ===")
    for i, data in ipairs(State.ApiCandidates) do
        if i > MAX_CANDIDATES then break end
        table.insert(lines, string.format("[API CANDIDATE %d] Name=%s | Class=%s | Path=%s", i, data.Name, data.ClassName, data.Path))
    end
    table.insert(lines, "")
    table.insert(lines, "=== ALL API OBJECTS ===")
    for i, data in ipairs(State.ApiResults) do
        table.insert(lines, string.format("[API %d] %s | %s | %s", i, data.Name, data.ClassName, data.Path))
    end
    return table.concat(lines, "\n")
end

local function CopyAll()
    local text = buildClipboardText()
    local clipboard = setclipboard or toclipboard or (syn and syn.write_clipboard)
    if type(clipboard) ~= "function" then
        Status.Text = "Status: clipboard indisponível neste ambiente"
        warn("[Nexus Inspector] Nenhuma função de clipboard disponível.")
        return
    end
    local ok, err = pcall(clipboard, text)
    if ok then
        Status.Text = "Status: tudo copiado para a área de transferência"
        CopyButton.Text = "COPIED!"
        task.delay(1.5, function() if CopyButton and CopyButton.Parent then CopyButton.Text = "COPY ALL" end end)
    else
        Status.Text = "Status: falha ao copiar"
        warn("[Nexus Inspector] Clipboard error:", err)
    end
end
...
CopyButton.Activated:Connect(CopyAll)
