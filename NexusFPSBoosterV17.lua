...
--==================================================
-- AIM LOCK - JJS HARD LOCK MAX+++ ADAPTIVE
--==================================================

-- Hard-lock design:
-- 1) Acquire exactly one target when the feature is enabled.
-- 2) Never scan for another target while the lock is active.
-- 3) Follow the target from live character data; no movement prediction.
-- 4) Screen/FOV/visibility do not break an already established lock.
-- 5) If the target dies, is replaced, or disappears, clear the lock only.
-- 6) The aim point is selected automatically from the current situation.
--    There is no manual torso/head toggle.
...
local LOCKON_DEADZONE_PIXELS = 0.65
local LOCKON_MAX_ANGLE_STEP = math.rad(180)

-- Adaptive aim point behavior. The point changes only when the situation
-- meaningfully changes, preventing rapid head/torso oscillation.
...
local function ComputeAdaptiveAimOffset()
	if not AimTargetRoot or not AimTargetHumanoid then
		return Vector3.new(0, AIMPOINT_GROUND_OFFSET, 0)
	end
	...
	-- Close targets: slightly lower center mass for a more stable lock.
	-- Far targets: slightly higher center mass for better visual centering.
	local height
...
local function UpdateAdaptiveAimPoint(force)
	...
	-- Hysteresis: do not switch aim modes repeatedly when a target is right
	-- around a distance/state boundary.
...
local function FindLockOnTarget()
	...
	-- The ONLY player scan in the lock cycle.
...
local function UpdateAimLock()
	...
	-- Small deadzone prevents micro-jitter when the target is already almost
	-- centered, while still allowing large/real movement to be corrected.
	...
	-- Avoid pathological one-frame camera jumps while retaining very fast
	-- tracking for dashes and spins.
	if angle > LOCKON_MAX_ANGLE_STEP then
		local axis = currentLook:Cross(desiredLook)
		if axis.Magnitude > 0.00001 then
			axis = axis.Unit
			local limited = CFrame.fromAxisAngle(axis, LOCKON_MAX_ANGLE_STEP)
			Camera.CFrame = CFrame.lookAt(cameraPosition, cameraPosition + limited:VectorToWorldSpace(currentLook))
			return
		end
	end
...
RunService:BindToRenderStep("NexusAimLock", LOCKON_CAMERA_PRIORITY, UpdateAimLock)
