...
--==================================================
-- NEXUS DUEL CAMERA / HARD LOCK - STUDIO V18
--==================================================
-- Para Place próprio no Roblox Studio.
-- Mantém um único alvo, enquadra jogador + alvo e usa highlights:
-- jogador = azul, alvo = roxo.
...
-- V18.2 GOD LOCK tuning: stable target acquisition, prediction and safer framing.
...
local LOCKON_DISTANCE_SMOOTH = 11
local LOCKON_MIN_DISTANCE = 9
local LOCKON_MAX_DISTANCE = 32
local LOCKON_DEFAULT_DISTANCE = 15
local LOCKON_VERTICAL_OFFSET = 3.8
local LOCKON_FRAME_MARGIN = 0.16
local LOCKON_MIN_TARGET_SCREEN = 0.10
local LOCKON_MAX_TARGET_SCREEN = 0.90
local LOCKON_WALL_BUFFER = 1.25

-- Higher, slightly wider framing keeps the local player inside the viewport.
local LOCKON_CAMERA_HEIGHT = 3.0
local LOCKON_LOOK_HEIGHT = 1.35
local LOCKON_EXTRA_DISTANCE = 4.5

-- Prediction is deliberately capped so latency compensation cannot over-lead.
local LOCKON_PREDICTION = true
local LOCKON_PREDICTION_TIME = 0.075
local LOCKON_MAX_PREDICTION = 5.0
local LOCKON_VELOCITY_SMOOTH = 10

-- Keep both characters away from the extreme edges of the screen.
local LOCKON_EDGE_MARGIN = 0.12
local LOCKON_REFRAME_SMOOTH = 14
...
local function GetLockOnPart(character)
	...
	-- Prefer the head for a more natural lock point, then fall back safely.
...
local function GetSafeCameraDistance(center, desiredDistance, direction, localRoot, targetRoot)
	local distance = math.clamp(desiredDistance, LOCKON_MIN_DISTANCE, LOCKON_MAX_DISTANCE)
	local origin = center
	local castDirection = -direction * distance
	local params = RaycastParams.new()
	params.FilterType = Enum.RaycastFilterType.Exclude
	params.FilterDescendantsInstances = {Player.Character, AimTarget}
	params.IgnoreWater = true
	local hit = Workspace:Raycast(origin, castDirection, params)
	if hit then
		return math.clamp(math.max(LOCKON_MIN_DISTANCE, (hit.Position - origin).Magnitude - LOCKON_WALL_BUFFER), LOCKON_MIN_DISTANCE, distance)
	end
	return distance
end
...
local function UpdateDuelCamera(dt)
	...
	-- Frame both characters, with a small bias toward the local player so they
	-- remain comfortably visible instead of drifting out of the viewport.
	...
	local fovFactor = math.tan(math.rad(math.clamp(Camera.FieldOfView, 45, 90)) * .5)
	local requiredDistance = (separation * .5) / math.max(fovFactor * (1 - LOCKON_FRAME_MARGIN), .2)
	local desiredDistance = math.clamp(requiredDistance + LOCKON_EXTRA_DISTANCE, LOCKON_MIN_DISTANCE, LOCKON_MAX_DISTANCE)

	local lookPoint = midpoint + Vector3.new(0, LOCKON_LOOK_HEIGHT, 0)

	-- If either character approaches a viewport edge, bias the framing back
	-- toward the safer center before applying the final camera transform.
	local localScreen, localOnScreen = Camera:WorldToViewportPoint(a)
	local targetScreen, targetOnScreen = Camera:WorldToViewportPoint(b)
	if localOnScreen and targetOnScreen then
		local viewport = Camera.ViewportSize
		local minX, maxX = viewport.X * LOCKON_EDGE_MARGIN, viewport.X * (1 - LOCKON_EDGE_MARGIN)
		local minY, maxY = viewport.Y * LOCKON_EDGE_MARGIN, viewport.Y * (1 - LOCKON_EDGE_MARGIN)
		local shiftX, shiftY = 0, 0
		for _, screen in ipairs({localScreen, targetScreen}) do
			if screen.X < minX then shiftX = math.max(shiftX, minX - screen.X) end
			if screen.X > maxX then shiftX = math.min(shiftX, maxX - screen.X) end
			if screen.Y < minY then shiftY = math.max(shiftY, minY - screen.Y) end
			if screen.Y > maxY then shiftY = math.min(shiftY, maxY - screen.Y) end
		end
		if math.abs(shiftX) > 0.5 or math.abs(shiftY) > 0.5 then
			local cameraRight = Camera.CFrame.RightVector
			local cameraUp = Camera.CFrame.UpVector
			local viewportScale = math.max(viewport.X, viewport.Y)
			local correction = (cameraRight * (shiftX / viewportScale) + cameraUp * (shiftY / viewportScale)) * LOCKON_REFRAME_SMOOTH
			lookPoint += correction
		end
	end

	local lookDirection = (lookPoint - Camera.CFrame.Position)
	if lookDirection.Magnitude < .001 then lookDirection = Camera.CFrame.LookVector end
	lookDirection = lookDirection.Unit

	local safeDistance = GetSafeCameraDistance(lookPoint, desiredDistance, lookDirection, localRoot, AimTargetRoot)
	...
	-- Keep the existing left/right side, but explicitly raise the camera.
	local currentOffset = Camera.CFrame.Position - midpoint
	local horizontalOffset = Vector3.new(currentOffset.X, 0, currentOffset.Z)
	local side
	if horizontalOffset.Magnitude > .1 then
		side = horizontalOffset.Unit
	else
		local fallback = Vector3.new(-Camera.CFrame.LookVector.X, 0, -Camera.CFrame.LookVector.Z)
		side = fallback.Magnitude > .001 and fallback.Unit or Vector3.new(0, 0, -1)
	end
	local desiredPosition = midpoint + side * LastCameraDistance + Vector3.new(0, LOCKON_CAMERA_HEIGHT, 0)
