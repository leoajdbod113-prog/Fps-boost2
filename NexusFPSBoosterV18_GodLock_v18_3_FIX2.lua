...
local function UpdateDuelCamera(dt)
	...
	local midpoint = (a + b) * .5

	local desiredDistance = math.clamp(GetRequiredDistance(Camera, a, b), LOCKON_MIN_DISTANCE, LOCKON_MAX_DISTANCE)
	local lookPoint = midpoint + Vector3.new(0, LOCKON_LOOK_HEIGHT, 0)
	local side = GetDesiredCameraSide(localRoot, AimTargetRoot, dt)

	-- A câmera começa atrás do movimento, mas o alvo ainda influencia a lateral.
	local desiredPosition = midpoint + side * desiredDistance + Vector3.new(0, LOCKON_CAMERA_HEIGHT, 0)
	...
	desiredPosition = midpoint + side * LastCameraDistance + Vector3.new(0, LOCKON_CAMERA_HEIGHT, 0)
	...
	if IsBadFrame(a) or IsBadFrame(b) then
		...
		desiredPosition = midpoint + side * emergencyDistance + Vector3.new(0, LOCKON_CAMERA_HEIGHT, 0)
