...
--==================================================
-- NEXUS DUEL CAMERA / HARD LOCK - STUDIO V18.3
--==================================================
-- V18.3: enquadramento adaptativo + camera side tracking.
...
local LOCKON_CAMERA_SMOOTH = 14
local LOCKON_DISTANCE_SMOOTH = 13
local LOCKON_SIDE_SMOOTH = 8
local LOCKON_MIN_DISTANCE = 10
local LOCKON_MAX_DISTANCE = 38
local LOCKON_DEFAULT_DISTANCE = 16
local LOCKON_VERTICAL_OFFSET = 3.8
local LOCKON_FRAME_MARGIN = 0.22
local LOCKON_WALL_BUFFER = 1.25

-- Camera mais alta, mas sem afastar demais.
local LOCKON_CAMERA_HEIGHT = 3.2
local LOCKON_LOOK_HEIGHT = 1.15
local LOCKON_EXTRA_DISTANCE = 5.0

-- Prediction limitada para não ultrapassar o alvo.
local LOCKON_PREDICTION = true
local LOCKON_PREDICTION_TIME = 0.075
local LOCKON_MAX_PREDICTION = 5.0

-- FOV safety: quanto maior, mais espaço de emergência.
local LOCKON_EDGE_MARGIN = 0.16
local LOCKON_EMERGENCY_MARGIN = 0.08
local LOCKON_MOVEMENT_THRESHOLD = 2.0
local LOCKON_DIRECTION_MEMORY = 0.82
...
local function GetPredictedTargetPosition()
	...
	if velocity.Magnitude < 0.05 then return position end
	local lead = velocity * math.clamp(LOCKON_PREDICTION_TIME, 0, 0.25)
	if lead.Magnitude > LOCKON_MAX_PREDICTION then lead = lead.Unit * LOCKON_MAX_PREDICTION end
...
local function GetSafeCameraDistance(center, desiredDistance, direction)
	local distance = math.clamp(desiredDistance, LOCKON_MIN_DISTANCE, LOCKON_MAX_DISTANCE)
	local params = RaycastParams.new()
	params.FilterType = Enum.RaycastFilterType.Exclude
	params.FilterDescendantsInstances = {Player.Character, AimTarget}
	params.IgnoreWater = true
	local hit = Workspace:Raycast(center, -direction * distance, params)
	if hit then
		return math.clamp(math.max(LOCKON_MIN_DISTANCE, (hit.Position - center).Magnitude - LOCKON_WALL_BUFFER), LOCKON_MIN_DISTANCE, distance)
	end
	return distance
end

-- Escolhe a direção da câmera com base no movimento atual.
-- Se o jogador andar para trás, o "atrás" da câmera acompanha essa nova direção.
local function GetDesiredCameraSide(localRoot, targetRoot, dt)
	local localVelocity = localRoot.AssemblyLinearVelocity
	local movement = Vector3.new(localVelocity.X, 0, localVelocity.Z)
	local movementDirection = nil

	if movement.Magnitude >= LOCKON_MOVEMENT_THRESHOLD then
		movementDirection = movement.Unit
		if LastMovementDirection then
			movementDirection = LastMovementDirection:Lerp(movementDirection, 1 - LOCKON_DIRECTION_MEMORY).Unit
		end
		LastMovementDirection = movementDirection
	elseif LastMovementDirection then
		movementDirection = LastMovementDirection
	end

	local awayFromTarget = localRoot.Position - targetRoot.Position
	awayFromTarget = Vector3.new(awayFromTarget.X, 0, awayFromTarget.Z)
	local targetAway = awayFromTarget.Magnitude > .001 and awayFromTarget.Unit or Vector3.new(0, 0, -1)

	-- Movimento domina enquanto o jogador estiver andando; parado, a câmera
	-- volta naturalmente para o lado oposto ao alvo.
	local desired
	if movementDirection then
		desired = (-movementDirection * .72 + targetAway * .28)
	else
		desired = targetAway
	end

	if desired.Magnitude < .001 then desired = targetAway end
	desired = desired.Unit

	if LastCameraSide and LastCameraSide:Dot(desired) < -0.75 then
		-- Evita uma inversão instantânea de 180 graus.
		LastCameraSide = LastCameraSide:Lerp(desired, math.clamp(dt * LOCKON_SIDE_SMOOTH, 0, 1)).Unit
	else
		local alpha = 1 - math.exp(-LOCKON_SIDE_SMOOTH * math.clamp(dt, 1/240, 1/20))
		LastCameraSide = LastCameraSide and LastCameraSide:Lerp(desired, alpha).Unit or desired
	end
	return LastCameraSide
end

local function GetRequiredDistance(camera, a, b)
	local separation = (b - a).Magnitude
	local fov = math.rad(math.clamp(camera.FieldOfView, 45, 90))
	local verticalFov = fov
	local horizontalFov = 2 * math.atan(math.tan(fov * .5) * math.max(camera.ViewportSize.X / math.max(camera.ViewportSize.Y, 1), 1))
	local usableVertical = math.max(math.tan(verticalFov * .5) * (1 - LOCKON_FRAME_MARGIN), .18)
	local usableHorizontal = math.max(math.tan(horizontalFov * .5) * (1 - LOCKON_FRAME_MARGIN), .18)
	local verticalDistance = separation * .5 / usableVertical
	local horizontalDistance = separation * .5 / usableHorizontal
	return math.max(verticalDistance, horizontalDistance) + LOCKON_EXTRA_DISTANCE
end
...
local function UpdateDuelCamera(dt)
	...
	-- PLAYER-SYNC: a câmera fica ancorada atrás do jogador. O alvo controla
	-- principalmente o ponto de visão, nunca a posição-base da câmera.
	local localHumanoid = localCharacter:FindFirstChildOfClass('Humanoid')
	if localHumanoid then
		pcall(function() localHumanoid.AutoRotate = true end)
	end
	RefreshSelfMarker()

	local predictedTarget = GetPredictedTargetPosition() or AimTargetRoot.Position
	local a = localRoot.Position + Vector3.new(0, LOCKON_VERTICAL_OFFSET, 0)
	local b = predictedTarget + Vector3.new(0, LOCKON_VERTICAL_OFFSET, 0)

	local moveDir = localHumanoid and localHumanoid.MoveDirection or Vector3.zero
	moveDir = Vector3.new(moveDir.X, 0, moveDir.Z)
	local travelDir
	if moveDir.Magnitude > LOCKON_MOVEMENT_THRESHOLD * 0.1 then
		travelDir = moveDir.Unit
	else
		local rootLook = Vector3.new(localRoot.CFrame.LookVector.X, 0, localRoot.CFrame.LookVector.Z)
		travelDir = rootLook.Magnitude > 0.001 and rootLook.Unit or Vector3.new(0, 0, -1)
	end

	-- Mantém a direção da câmera estável, evitando inversões cinematográficas.
	LastCameraSide = LastCameraSide and LastCameraSide:Lerp(-travelDir, 1 - math.exp(-18 * math.clamp(dt, 1/240, 1/20))) or -travelDir
	if LastCameraSide.Magnitude < 0.001 then LastCameraSide = -travelDir end
	LastCameraSide = LastCameraSide.Unit

	local separation = (b - a).Magnitude
	local desiredDistance = math.clamp(GetRequiredDistance(Camera, a, b), LOCKON_MIN_DISTANCE, LOCKON_MAX_DISTANCE)
	local targetBias = math.clamp(separation / 35, 0, 1)
	local lookPoint = a:Lerp(b, 0.38 + 0.22 * targetBias) + Vector3.new(0, LOCKON_LOOK_HEIGHT, 0)

	-- Posição sempre relativa ao jogador: atrás + altura.
	local desiredPosition = localRoot.Position + LastCameraSide * desiredDistance + Vector3.new(0, LOCKON_CAMERA_HEIGHT, 0)
	local lookDirection = (lookPoint - desiredPosition)
	if lookDirection.Magnitude < .001 then lookDirection = Camera.CFrame.LookVector end
	lookDirection = lookDirection.Unit

	local safeDistance = GetSafeCameraDistance(lookPoint, desiredDistance, lookDirection)
	local distAlpha = 1 - math.exp(-LOCKON_DISTANCE_SMOOTH * math.clamp(dt, 1/240, 1/20))
	LastCameraDistance = LastCameraDistance + (safeDistance - LastCameraDistance) * distAlpha

	desiredPosition = localRoot.Position + LastCameraSide * LastCameraDistance + Vector3.new(0, LOCKON_CAMERA_HEIGHT, 0)

	-- Segunda proteção: se qualquer ponto estiver fora da tela, aumenta a distância
	-- e recentraliza. Isso corrige o caso do vídeo em que o alvo ficava na tela
	-- enquanto o próprio jogador saía do enquadramento.
	local emergencyDistance = LastCameraDistance
	local viewport = Camera.ViewportSize
	local function IsBadFrame(position)
		local screen, onScreen = Camera:WorldToViewportPoint(position)
		if not onScreen or screen.Z <= 0 then return true end
		local marginX = viewport.X * LOCKON_EMERGENCY_MARGIN
		local marginY = viewport.Y * LOCKON_EMERGENCY_MARGIN
		return screen.X < marginX or screen.X > viewport.X - marginX
			or screen.Y < marginY or screen.Y > viewport.Y - marginY
	end

	if IsBadFrame(a) or IsBadFrame(b) then
		emergencyDistance = math.min(LOCKON_MAX_DISTANCE, LastCameraDistance + 5)
		desiredPosition = localRoot.Position + LastCameraSide * emergencyDistance + Vector3.new(0, LOCKON_CAMERA_HEIGHT, 0)
		lookPoint = midpoint + Vector3.new(0, LOCKON_LOOK_HEIGHT, 0)
		LastCameraDistance = emergencyDistance
	end

	local posAlpha = 1 - math.exp(-LOCKON_CAMERA_SMOOTH * math.clamp(dt, 1/240, 1/20))
	LastCameraPosition = LastCameraPosition and LastCameraPosition:Lerp(desiredPosition, posAlpha) or desiredPosition
	local desiredCFrame = CFrame.lookAt(LastCameraPosition, lookPoint)
	Camera.CFrame = Camera.CFrame:Lerp(desiredCFrame, posAlpha)
...
if not __NexusRunOk then
	...
	warn("[NEXUS V18.3 FIX2] " .. tostring(__NexusRunErr))
else
	NexusBootStatus("GUI criada • V18.3 FIX2")
	task.delay(2, function()
		pcall(function() NexusBootGui:Destroy() end)
	end)
end
