...
local function FaceLocalCharacterToTarget(dt)
	...
	local speed = math.clamp(flat.Magnitude * 1.8, 10, 28)
...
local function UpdateDuelCamera(dt)
	...
	-- O alvo controla o olhar; a posição da câmera continua presa ao player.
	local lookPoint = targetFocus
	local desiredCFrame = CFrame.lookAt(desiredPosition, lookPoint)

	-- Suavização adaptativa: mais rápida quando o player está se movendo,
	-- mais suave quando está parado. Isso reduz atraso e sensação de câmera pesada.
	local velocity = Vector3.new(localRoot.AssemblyLinearVelocity.X, 0, localRoot.AssemblyLinearVelocity.Z).Magnitude
	local cameraSmooth = math.clamp(18 + velocity * 0.35, 18, 30)
