...
local LOCKON_FACING_SMOOTH = 10
local LOCKON_CAMERA_SMOOTH = 12
local LOCKON_DISTANCE_SMOOTH = 9
local LOCKON_SIDE_SMOOTH = 8
local LOCKON_MIN_DISTANCE = 11
local LOCKON_MAX_DISTANCE = 42
local LOCKON_DEFAULT_DISTANCE = 18
...
local LOCKON_CAMERA_HEIGHT = 4.2
local LOCKON_LOOK_HEIGHT = 1.35
local LOCKON_EXTRA_DISTANCE = 6.0
...
local LOCKON_TARGET_AIM_BLEND = 0.82
local LOCKON_PLAYER_AIM_DISTANCE = 5.5
local LOCKON_MAX_TURN_RATE = 16

-- V18.8: rotação orbital assistida em aproximações/dashes.
-- Não teleporta o player; apenas orienta a rotação para acompanhar o alvo
-- e, quando há deslocamento rápido perto dele, usa uma tangente orbital.
local LOCKON_ORBIT_ENABLED = true
local LOCKON_ORBIT_DISTANCE = 13
local LOCKON_ORBIT_SPEED = 24
local LOCKON_ORBIT_TARGET_BLEND = 0.72
local LOCKON_ORBIT_TANGENT_BLEND = 0.62
local LOCKON_ORBIT_TURN_RATE = 22
local LOCKON_ORBIT_MEMORY = 0.90
local OrbitSignMemory = 1
...
local PreviousLocalAutoRotate = nil
local CameraRaycastClock = 0
local CachedSafeDistance = LOCKON_DEFAULT_DISTANCE
local LocalDeathConnection = nil
...
local function FaceLocalCharacterToTarget(dt)
	...
	local flat = Vector3.new(predicted.X - localRoot.Position.X, 0, predicted.Z - localRoot.Position.Z)
	if flat.Magnitude < 0.001 then return end

	pcall(function() localHumanoid.AutoRotate = false end)
	LockedHumanoidForFacing = localHumanoid

	-- Rotação mais solta: normalmente encara o alvo.
	local desiredDirection = flat.Unit

	-- V18.8: quando o player está muito perto e se deslocando rápido (dash/avanço),
	-- usa uma direção tangencial para criar o efeito de “girar em volta” do inimigo.
	-- O movimento/posição continuam sendo controlados pelo jogo; aqui só ajustamos a orientação.
	if LOCKON_ORBIT_ENABLED and flat.Magnitude <= LOCKON_ORBIT_DISTANCE then
		local horizontalVelocity = Vector3.new(localRoot.AssemblyLinearVelocity.X, 0, localRoot.AssemblyLinearVelocity.Z)
		local speedMagnitude = horizontalVelocity.Magnitude
		if speedMagnitude >= LOCKON_ORBIT_SPEED then
			local radial = flat.Unit
			local tangentA = Vector3.new(-radial.Z, 0, radial.X)
			local tangentB = -tangentA
			local movement = horizontalVelocity.Unit

			local scoreA = movement:Dot(tangentA)
			local scoreB = movement:Dot(tangentB)
			local wantedSign = (scoreA >= scoreB) and 1 or -1
			OrbitSignMemory = (OrbitSignMemory * LOCKON_ORBIT_MEMORY) + (wantedSign * (1 - LOCKON_ORBIT_MEMORY))
			local orbitSign = OrbitSignMemory >= 0 and 1 or -1
			local tangent = orbitSign > 0 and tangentA or tangentB

			local orbitDirection = (flat.Unit * LOCKON_ORBIT_TARGET_BLEND + tangent * LOCKON_ORBIT_TANGENT_BLEND)
			if orbitDirection.Magnitude > 0.001 then
				desiredDirection = orbitDirection.Unit
			end
		end
	end

	local desired = CFrame.lookAt(localRoot.Position, localRoot.Position + desiredDirection)
	local speed = math.clamp(LOCKON_FACING_SMOOTH + flat.Magnitude * 0.10, 8, LOCKON_MAX_TURN_RATE)
	if flat.Magnitude <= LOCKON_ORBIT_DISTANCE and localRoot.AssemblyLinearVelocity.Magnitude >= LOCKON_ORBIT_SPEED then
		speed = LOCKON_ORBIT_TURN_RATE
	end
	local alpha = 1 - math.exp(-speed * math.clamp(dt, 1/240, 1/20))
...
local function GetSafeCameraDistance(origin, desiredPosition, desiredDistance, dt)
...
local function UpdateDuelCamera(dt)
	...
	-- 1) PRIMEIRO o player acompanha o alvo.
	FaceLocalCharacterToTarget(dt)

	-- 2) DEPOIS a câmera acompanha o player. O alvo só define o ponto de visão.
	...
	-- Distância adaptativa, mas sem o efeito cinematográfico de afastar demais.
	local desiredDistance = math.clamp(LOCKON_DEFAULT_DISTANCE + math.clamp(separation - 12, 0, 18) * 0.22,
		LOCKON_MIN_DISTANCE, math.min(LOCKON_MAX_DISTANCE, 30))

	-- A câmera fica SEMPRE atrás do player.
	local desiredPosition = rootPos - rootForward * desiredDistance + Vector3.new(0, LOCKON_CAMERA_HEIGHT, 0)
	...
	desiredPosition = rootPos - rootForward * LastCameraDistance + Vector3.new(0, LOCKON_CAMERA_HEIGHT, 0)

	-- O alvo continua sendo a referência principal, mas o olhar fica um pouco mais solto.
	-- Isso evita a sensação de câmera “grudada” no inimigo.
	local playerLookPoint = playerFocus + rootForward * LOCKON_PLAYER_AIM_DISTANCE
	local lookPoint = playerLookPoint:Lerp(targetFocus, LOCKON_TARGET_AIM_BLEND)
	local desiredCFrame = CFrame.lookAt(desiredPosition, lookPoint)

	-- Suavização adaptativa: responde mais quando há movimento, sem ficar nervosa.
	local velocity = Vector3.new(localRoot.AssemblyLinearVelocity.X, 0, localRoot.AssemblyLinearVelocity.Z).Magnitude
	local cameraSmooth = math.clamp(12 + velocity * 0.28, 12, 22)
