...
local LOCKON_FACING_SMOOTH = 14
local LOCKON_CAMERA_SMOOTH = 9
local LOCKON_DISTANCE_SMOOTH = 8
local LOCKON_MIN_DISTANCE = 9
local LOCKON_MAX_DISTANCE = 24
local LOCKON_DEFAULT_DISTANCE = 13
local LOCKON_VERTICAL_OFFSET = 3.2
local LOCKON_FRAME_MARGIN = 0.16
local LOCKON_MIN_TARGET_SCREEN = 0.18
local LOCKON_MAX_TARGET_SCREEN = 0.82
...
local function UpdateDuelCamera(dt)
	...
	local b = AimTargetRoot.Position + Vector3.new(0, LOCKON_VERTICAL_OFFSET, 0)
	...
	local desiredDistance = math.clamp(requiredDistance + 2.5, LOCKON_MIN_DISTANCE, LOCKON_MAX_DISTANCE)

	local lookDirection = (midpoint - Camera.CFrame.Position)
	if lookDirection.Magnitude < .001 then lookDirection = Camera.CFrame.LookVector end
	lookDirection = lookDirection.Unit

	local safeDistance = GetSafeCameraDistance(midpoint, desiredDistance, lookDirection, localRoot, AimTargetRoot)
	...
	-- Preserve the current side of the duel camera while aiming at the midpoint.
	local currentOffset = Camera.CFrame.Position - midpoint
	local side = currentOffset.Magnitude > .1 and currentOffset.Unit or -Camera.CFrame.LookVector
	local desiredPosition = midpoint + side * LastCameraDistance
	...
	local desiredCFrame = CFrame.lookAt(LastCameraPosition, midpoint)
