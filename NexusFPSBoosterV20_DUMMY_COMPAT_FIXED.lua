...
-- Dummy/NPC compatibility for the user's own test arena.
-- Any Workspace Model with a live Humanoid + HumanoidRootPart can be targeted.
LOCKON_ALLOW_DUMMIES = true
LOCKON_DUMMY_SEARCH_DEPTH = true
...
local function IsValidAimModel(model)
	if not model or not model:IsA('Model') then return false end
	if Player and Player.Character and model == Player.Character then return false end
...
local function IsPlayerCharacter(model)
	for _, other in ipairs(Players:GetPlayers()) do
		if other ~= Player and other.Character == model then
			return true
		end
	end
	return false
...
local function GetAimTargetModels()
	local models = {}
	local seen = {}

	-- Player characters remain supported exactly as before.
	for _, other in ipairs(Players:GetPlayers()) do
		if other ~= Player and other.Character and not seen[other.Character] then
			local valid = IsValidAimModel(other.Character)
			if valid then
	...
	-- Test-arena Dummy/NPC support. We do not assume a specific Dummy name.
	if LOCKON_ALLOW_DUMMIES and LOCKON_DUMMY_SEARCH_DEPTH then
		local ok, descendants = pcall(function()
			return Workspace:GetDescendants()
		end)
		if ok and descendants then
			for _, instance in ipairs(descendants) do
				if instance:IsA('Model') and not seen[instance] and not IsPlayerCharacter(instance) then
					local valid = IsValidAimModel(instance)
					if valid then
						seen[instance] = true
...
local function FindBestAimTarget()
	...
	local bestTarget, bestRoot, bestScore = nil, nil, math.huge

	for _, model in ipairs(GetAimTargetModels()) do
		local valid, humanoid, root = IsValidAimModel(model)
		if valid then
			local offset = root.Position - localRoot.Position
			local distance = offset.Magnitude
			if distance > 0.001 and distance <= LOCKON_RANGE and distance <= LOCKON_MAX_FOLLOW_DISTANCE then
...
function IsCurrentTargetDummy()
	if not IsCurrentTargetValid() then return false end
	return not IsPlayerCharacter(AimTarget)
end

function GetAimTarget()
	if not IsCurrentTargetValid() then return nil end
	return AimTarget
end
...
local NexusV4_BlockUnblockAdapter = {
	block = function()
		...
		-- Fallback for the known test-arena UI path discovered by the inspector.
		-- We only invoke the legitimate GuiButton; we do not fire remotes or invent an API.
		local ok, result = pcall(function()
			local playerGui = Player and Player:FindFirstChildOfClass('PlayerGui')
			local controls = playerGui and playerGui:FindFirstChild('Controls')
			local mobile = controls and controls:FindFirstChild('Mobile')
			local jump = mobile and mobile:FindFirstChild('Jump')
			local blockButton = jump and jump:FindFirstChild('Block')
			if not blockButton or not blockButton:IsA('GuiButton') then
				error('Controls.Mobile.Jump.Block GuiButton not found')
			end
			blockButton:Activate()
			return true
		end)
		if not ok then warn('[NEXUS V4] Block UI fallback error:', result) end
	...
	unblock = function()
		...
		-- Same legitimate UI adapter is used for the release/toggle path.
		local ok, result = pcall(function()
			local playerGui = Player and Player:FindFirstChildOfClass('PlayerGui')
			local controls = playerGui and playerGui:FindFirstChild('Controls')
			local mobile = controls and controls:FindFirstChild('Mobile')
		...
		if not ok then warn('[NEXUS V4] Unblock UI fallback error:', result) end
		return ok and result == true
	end,
}
...
_G.NexusV4_AutoBlock = {
	...
	GetTarget = function() return GetAimTarget() end,
	IsTargetDummy = function() return IsCurrentTargetDummy() end,
