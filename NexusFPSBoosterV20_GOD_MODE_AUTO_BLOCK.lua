...
local AUTO_BLOCK_DISTANCE = 25
local AUTO_BLOCK_DASH_DISTANCE = 32
local AUTO_BLOCK_MIN_APPROACH_SPEED = 12
local AUTO_BLOCK_DASH_SPEED = 42
local AUTO_BLOCK_REACTION_TIME = 0.12
local AUTO_BLOCK_HOLD_TIME = 0.22
local AUTO_BLOCK_RELEASE_COOLDOWN = 0.16
local AUTO_BLOCK_INPUT_PRIORITY = 3000

local AutoBlockActive = false
local AutoBlockThreat = 0
local AutoBlockLastStart = 0
local AutoBlockLastRelease = 0
local AutoBlockUntil = 0
local AutoBlockBound = false
local AutoBlockVirtualInput = nil
local AutoBlockTouchButton = nil

local GOD_MAX_TARGET_DISTANCE = 500
local GOD_PREDICTION_MIN = 0.055
local GOD_PREDICTION_MAX = 0.18
local GOD_PREDICTION_SPEED_SCALE = 0.0022
local GOD_ACCEL_RESPONSE = 0.18
local GOD_FACING_MOVE_BLEND = 0.30
local GOD_FACING_DASH_BLEND = 0.12
local GOD_DASH_SPEED_THRESHOLD = 42
local GOD_CAMERA_HEIGHT_MIN = 4.2
local GOD_CAMERA_HEIGHT_MAX = 5.4
local GOD_CAMERA_DISTANCE_MIN = 12
local GOD_CAMERA_DISTANCE_MAX = 36
local GOD_FOV_NORMAL = 70
local GOD_FOV_COMBAT = 76
local GOD_FOV_DASH = 82
local GOD_FOV_SMOOTH = 10
...
local TargetCharacterRemovingConnection = nil
local LastTargetVelocity = Vector3.zero
local TargetAcceleration = Vector3.zero
local LastLocalVelocity = Vector3.zero
local CombatFOV = GOD_FOV_NORMAL
local PreviousCameraFOV = nil
local GodCoreActive = false
...
local function SendBlockInput(isDown)
...
local function SetAutoBlockState(active)
...
local function BindAutoBlockInput()
...
local function UnbindAutoBlockInput()
	AutoBlockBound = false
	SetAutoBlockState(false)
end

local function GetAutoBlockThreat()
...
local function FaceTargetForAutoBlock(dt)
	local character = Player.Character
...
local function UpdateAutoBlock(dt)
...
local GodModeBadge = Instance.new("TextLabel")
...
local AutoBlockBadge = Instance.new("TextLabel")
