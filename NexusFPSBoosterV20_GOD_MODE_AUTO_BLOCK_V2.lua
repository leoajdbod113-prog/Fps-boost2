...
function BindAutoBlockInput()
	-- O Auto Block nao captura o F do jogador.
	-- Ele somente envia F quando uma ameaca real e detectada.
	AutoBlockBound = true
end
...
-- Teclado/controle: evita que o Auto Block reaja ao ataque do proprio jogador.
-- Touch generico nao e capturado porque no mobile o mesmo evento pode ser usado pelo HUD.
...
function GetAutoBlockThreat()
	...
	-- Somente conta como ameaca quando o inimigo esta realmente vindo para nos.
	local approach = 0
	...
	if approach >= 0.45 and targetSpeed >= AUTO_BLOCK_MIN_APPROACH_SPEED then
		approachThreat = math.clamp((approach - 0.45) / 0.55, 0, 1)
	end

	local dashThreat = 0
	if targetSpeed >= AUTO_BLOCK_DASH_SPEED and approach >= 0.45 then
		dashThreat = math.clamp((targetSpeed - AUTO_BLOCK_DASH_SPEED) / 35, 0, 1)
	end

	-- Nao ativa apenas porque o inimigo esta perto. Ele precisa estar entrando.
	local threat = approachThreat * 0.55 + dashThreat * 0.95
	if distance <= AUTO_BLOCK_DISTANCE and approach >= 0.35 and targetSpeed >= AUTO_BLOCK_MIN_APPROACH_SPEED then
...
function UpdateAutoBlock(dt)
	...
	local character = Player.Character
	local localRoot = character and character:FindFirstChild('HumanoidRootPart')
	if not localRoot or not AimTargetRoot then
		if AutoBlockActive then SetAutoBlockState(false) end
		return
	end
	...
	-- O ataque do proprio jogador tem prioridade: nunca inicia Auto Block por cima dele.
	if IsLocalAttacking() then
		AutoBlockThreat = 0
		if AutoBlockActive then SetAutoBlockState(false) end
		return
	...
	-- Se o jogador apertou o botao de liberar, segura desarmado ate o alvo sair da zona.
	if AutoBlockManualRelease then
		AutoBlockThreat = 0
		if AutoBlockActive then SetAutoBlockState(false) end
	...
	if threat >= 0.72 then
		-- Reacao imediata: vira para o inimigo e segura o block enquanto ele estiver ameacando.
		FaceTargetForAutoBlock(dt)
		if not AutoBlockActive then
			SetAutoBlockState(true)
		end
	elseif AutoBlockActive then
		-- Sem cooldown: so libera quando o inimigo realmente sai da zona de seguranca.
		if distance > AUTO_BLOCK_RELEASE_DISTANCE then
			SetAutoBlockState(false)
		end
	end
...
AutoBlockReleaseButton.Activated:Connect(function()
	-- Clique manual desarma o Auto Block ate o inimigo sair da zona.
	AutoBlockManualRelease = true
	AutoBlockThreat = 0
	SetAutoBlockState(false)
