...
--==================================================
-- V20.2 SETTINGS / PROFILES / DIAGNOSTICS
--==================================================
...
local function NexusMakeButton(parent, text, x, y, w, callback)
...
local function NexusUpdateDiagnostics()
	...
	local connCount = 0
	for _, list in pairs(PlayerAuraConnections or {}) do
		connCount += #(list or {})
	end
	local mem = "N/A"
	...
	NexusDiagnosticsLabel.Text =
		"DIAGNOSTIC\n\n" ..
		"GUI: " .. (Gui and Gui.Parent and "OK" or "OFF") .. "\n" ..
		"Character: " .. (character and "READY" or "NONE") .. "\n" ..
		"Humanoid: " .. (humanoid and "READY" or "NONE") .. "\n" ..
		"Root: " .. (root and "READY" or "NONE") .. "\n" ..
		"Kamui: " .. (ObitoPhaseActive and "ACTIVE" or "READY") .. "\n" ..
		"Auras: " .. tostring(auraCount) .. "\n" ..
		"Aura connections: " .. tostring(connCount) .. "\n" ..
		"FPS: " .. tostring(FPS) .. "\n" ..
		"RAM: " .. tostring(mem) .. "\n" ..
...
local function NexusToggleSettings()
	...
	if NexusSettingsFrame then NexusSettingsFrame.Visible = NexusSettingsOpen end
end
...
NexusSettingsFrame = Instance.new("Frame")
NexusSettingsFrame.Name = "NexusSettings"
NexusSettingsFrame.Size = UDim2.new(1, -30, 1, -100)
NexusSettingsFrame.Position = UDim2.fromOffset(15, 98)
NexusSettingsFrame.BackgroundColor3 = Color3.fromRGB(12, 8, 18)
NexusSettingsFrame.BackgroundTransparency = 0.05
...
settingsTitle.Size = UDim2.new(1, -20, 0, 26)
settingsTitle.Position = UDim2.fromOffset(10, 8)
settingsTitle.BackgroundTransparency = 1
settingsTitle.Text = "N E X U S  //  SETTINGS"
settingsTitle.TextColor3 = WHITE
settingsTitle.TextSize = 14
settingsTitle.Font = Enum.Font.GothamBold
settingsTitle.TextXAlignment = Enum.TextXAlignment.Left
settingsTitle.ZIndex = 101
settingsTitle.Parent = NexusSettingsFrame

NexusMakeButton(NexusSettingsFrame, "NEXUS", 10, 42, 105, function()
	ApplyNexusTheme("NEXUS"); PersistConfig()
end).ZIndex = 101
NexusMakeButton(NexusSettingsFrame, "AKATSUKI", 120, 42, 105, function()
	ApplyNexusTheme("AKATSUKI"); PersistConfig()
end).ZIndex = 101
NexusMakeButton(NexusSettingsFrame, "OBITO", 230, 42, 105, function()
	ApplyNexusTheme("OBITO"); PersistConfig()
end).ZIndex = 101
NexusMakeButton(NexusSettingsFrame, "CYBER", 340, 42, 105, function()
	ApplyNexusTheme("CYBER"); PersistConfig()
end).ZIndex = 101

NexusMakeButton(NexusSettingsFrame, "AURA ON/OFF", 10, 80, 145, function()
	PlayerAuraEnabled = not PlayerAuraEnabled
	PersistConfig()
	PlayerAuraRefreshAll()
end).ZIndex = 101

NexusMakeButton(NexusSettingsFrame, "AURA +50", 165, 80, 100, function()
	PlayerAuraMaxDistance = math.clamp(PlayerAuraMaxDistance + 50, 25, 1000)
	PersistConfig()
end).ZIndex = 101

NexusMakeButton(NexusSettingsFrame, "AURA -50", 275, 80, 100, function()
	PlayerAuraMaxDistance = math.clamp(PlayerAuraMaxDistance - 50, 25, 1000)
	PersistConfig()
end).ZIndex = 101

NexusMakeButton(NexusSettingsFrame, "KAMUI ON/OFF", 10, 118, 145, function()
	OBITO_PHASE_ENABLED = not OBITO_PHASE_ENABLED
	if not OBITO_PHASE_ENABLED then ObitoPhaseOff("settings_disabled") end
	PersistConfig()
end).ZIndex = 101

NexusMakeButton(NexusSettingsFrame, "KAMUI DURATION +", 165, 118, 145, function()
	OBITO_PHASE_DURATION = math.clamp(OBITO_PHASE_DURATION + 0.05, 0.05, 2)
	PersistConfig()
end).ZIndex = 101

NexusMakeButton(NexusSettingsFrame, "KAMUI DURATION -", 320, 118, 145, function()
	OBITO_PHASE_DURATION = math.clamp(OBITO_PHASE_DURATION - 0.05, 0.05, 2)
	PersistConfig()
end).ZIndex = 101

NexusMakeButton(NexusSettingsFrame, "PERFIL PERFORMANCE", 10, 156, 145, function()
	NexusSaveProfile("PERFORMANCE")
end).ZIndex = 101
NexusMakeButton(NexusSettingsFrame, "PERFIL OBITO", 165, 156, 125, function()
	NexusSaveProfile("OBITO")
end).ZIndex = 101
NexusMakeButton(NexusSettingsFrame, "PERFIL NEXUS", 300, 156, 125, function()
	NexusSaveProfile("NEXUS")
end).ZIndex = 101

NexusMakeButton(NexusSettingsFrame, "CARREGAR PERFORMANCE", 10, 194, 180, function()
	NexusLoadProfile("PERFORMANCE")
end).ZIndex = 101
NexusMakeButton(NexusSettingsFrame, "CARREGAR OBITO", 200, 194, 145, function()
	NexusLoadProfile("OBITO")
end).ZIndex = 101
NexusMakeButton(NexusSettingsFrame, "CARREGAR NEXUS", 355, 194, 110, function()
	NexusLoadProfile("NEXUS")
end).ZIndex = 101

local compact = NexusMakeButton(NexusSettingsFrame, "MODO COMPACTO", 10, 232, 145, function()
	Main.Size = UDim2.fromOffset(500, 380)
end)
compact.ZIndex = 101

local normal = NexusMakeButton(NexusSettingsFrame, "MODO NORMAL", 165, 232, 145, function()
	Main.Size = FullSize
end)
normal.ZIndex = 101

local diag = NexusMakeButton(NexusSettingsFrame, "ATUALIZAR DIAGNÓSTICO", 320, 232, 145, function()
	NexusUpdateDiagnostics()
end)
diag.ZIndex = 101

NexusDiagnosticsLabel = Instance.new("TextLabel")
NexusDiagnosticsLabel.Size = UDim2.new(1, -20, 1, -275)
NexusDiagnosticsLabel.Position = UDim2.fromOffset(10, 270)
NexusDiagnosticsLabel.BackgroundTransparency = 1
...
-- Apply saved UI settings after controls exist.
ApplyNexusTheme(ThemeName)
ApplyUITransparency(UITransparency)

-- Optional FPS cap for environments exposing setfpscap.
FPS_CAP = tonumber(SavedConfig and SavedConfig.FPS_CAP) or 0
local function NexusApplyFPSCap()
	pcall(function()
		if type(setfpscap) == "function" and FPS_CAP > 0 then
			setfpscap(FPS_CAP)
		end
	end)
end
...
-- Auto performance: if FPS stays low, reduce expensive visuals without touching gameplay systems.
	...
	while Gui and Gui.Parent do
		...
		if NexusAutoPerformance and FPS > 0 then
			...
			if NexusLowFPSStreak >= 2 and BoosterEnabled and CurrentMode ~= "BATATA" then
				CurrentMode = "BATATA"
				pcall(function() ApplyMode(CurrentMode) end)
				NexusLowFPSStreak = 0
			end
		end
...
-- Respawn/reload safety for visual state.
Player.CharacterAdded:Connect(function()
	task.defer(function()
		NexusKamuiEffectStop()
