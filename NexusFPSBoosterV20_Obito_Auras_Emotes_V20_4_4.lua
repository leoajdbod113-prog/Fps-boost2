-- NEXUS V20.4.4 GOD MODE + AUTO BLOCK FIX FINAL
...
NexusBootLabel.Text = "NEXUS V20.4.4 • iniciando..."
...
local function NexusBootStatus(msg)
	pcall(function()
		NexusBootLabel.Text = "NEXUS V20.4.4 • " .. tostring(msg)
	end)
end
...
--==================================================
-- NEXUS THEMES / UI TRANSPARENCY
--==================================================
NexusThemes = {
	NEXUS = {Primary=Color3.fromRGB(180,60,255), Panel=Color3.fromRGB(30,18,45), Bg=Color3.fromRGB(15,8,22), Accent=Color3.fromRGB(240,235,250)},
	AKATSUKI = {Primary=Color3.fromRGB(210,35,45), Panel=Color3.fromRGB(42,12,18), Bg=Color3.fromRGB(14,7,9), Accent=Color3.fromRGB(255,225,225)},
	OBITO = {Primary=Color3.fromRGB(95,70,190), Panel=Color3.fromRGB(20,17,42), Bg=Color3.fromRGB(8,8,18), Accent=Color3.fromRGB(220,215,255)},
	CYBER = {Primary=Color3.fromRGB(30,220,210), Panel=Color3.fromRGB(10,32,35), Bg=Color3.fromRGB(5,12,14), Accent=Color3.fromRGB(225,255,255)},
}

function ApplyNexusTheme(name)
	ThemeName = NexusThemes[name] and name or "NEXUS"
	local t = NexusThemes[ThemeName]
	PURPLE, PANEL2, WHITE = t.Primary, t.Panel, t.Accent
	pcall(function()
		if Main then Main.BackgroundColor3=t.Bg; Main.BackgroundTransparency=UITransparency; MainStroke.Color=t.Primary end
		if Title then Title.TextColor3=t.Accent end
		if BoostTab then BoostTab.BackgroundColor3=t.Primary end
		if AimLockStroke then AimLockStroke.Color=t.Primary end
		if FlyStroke then FlyStroke.Color=t.Primary end
		if AimLockSelfMarker then AimLockSelfMarker.OutlineColor=t.Primary end
	end)
end

function ApplyUITransparency(value)
	UITransparency = math.clamp(tonumber(value) or UITransparency, 0, 0.75)
	pcall(function() if Main then Main.BackgroundTransparency=UITransparency end end)
end
...
ThemeName = "NEXUS"
UITransparency = 0.25
PlayerAuraMaxDistance = 250
PlayerAuraEnabled = true
PlayerAuraColorMode = "AUTO"
SpeedEnabled = false
SpeedValue = 20
...
FlySpeed = math.clamp(SavedConfig.FlySpeed or 60, 20, 150)
	ThemeName = SavedConfig.ThemeName or ThemeName
	UITransparency = math.clamp(tonumber(SavedConfig.UITransparency) or UITransparency, 0, 0.75)
	PlayerAuraMaxDistance = math.clamp(tonumber(SavedConfig.PlayerAuraMaxDistance) or PlayerAuraMaxDistance, 25, 1000)
	PlayerAuraEnabled = SavedConfig.PlayerAuraEnabled ~= false
	PlayerAuraColorMode = SavedConfig.PlayerAuraColorMode or PlayerAuraColorMode
end
...
function PersistConfig()
	SaveConfig({
		...
		PlayerAuraMaxDistance = PlayerAuraMaxDistance,
		PlayerAuraEnabled = PlayerAuraEnabled,
		PlayerAuraColorMode = PlayerAuraColorMode,
		ObitoDuration = OBITO_PHASE_DURATION,
		ObitoCooldown = OBITO_PHASE_COOLDOWN,
		ObitoEnabled = OBITO_PHASE_ENABLED,
		FPS_CAP = FPS_CAP or 0,
		NexusAutoPerformance = NexusAutoPerformance == true,
		Profiles = NexusProfiles or {},
		FPS_CAP = FPS_CAP or 0,
		NexusAutoPerformance = NexusAutoPerformance == true,
		NexusKamuiVisualEnabled = false
	})
end
...
local function PlayerAuraApply(player)
	if not PLAYER_AURA_ENABLED or not PlayerAuraEnabled or not player then return end
	local character = player.Character
	...
	if localRoot and targetRoot and (localRoot.Position - targetRoot.Position).Magnitude > PlayerAuraMaxDistance then
		PlayerAuraDestroy(player)
		return
	end
	...
	local color
	if PlayerAuraColorMode == 'PURPLE' then
		color = PLAYER_AURA_PURPLE
	elseif PlayerAuraColorMode == 'YELLOW' then
		color = PLAYER_AURA_YELLOW
	elseif PlayerAuraColorMode == 'BLUE' then
...
ObitoPhaseOriginal = {}
ObitoPhaseDamageTaken = 0
ObitoPhaseLastReason = ""
    ...
    for _, o in ipairs(c:GetDescendants()) do
        if o:IsA("BasePart") then
            if active then
                ...
                o.CanCollide = false
                -- Kamui visual: semi-transparent body only.
                -- Material and original color are preserved.
...
local function ObitoPhaseOff(reason)
    ...
    local h=c and c:FindFirstChildOfClass("Humanoid")
    local restoreHealth=ObitoPhaseHealthBefore
    ObitoPhaseLastReason = tostring(reason or "unknown")
    if c then ObitoSetPhaseVisual(c,false) end
    NexusKamuiEffectStop()

    -- BUGFIX: restaura o HP salvo mesmo quando o dano chegou no fim da janela
    -- ou quando o servidor/arena sobrescreveu a primeira tentativa.
    if h and restoreHealth and h.Health>0 and h.Health<restoreHealth then
        local target=math.min(restoreHealth,h.MaxHealth)
        for _=1,3 do
            pcall(function() h.Health=target end)
            if h.Health>=target then break end
            task.wait()
        end
    end
...
--==================================================
-- V20.2 KAMUI VISUAL EFFECT
-- Efeito visual local: vortex, blur, FOV e color correction.
-- Não depende de asset externo.
--==================================================
NexusKamuiFxGui = nil
NexusKamuiFxConnection = nil
NexusKamuiBlur = nil
NexusKamuiColor = nil
NexusKamuiBaseFov = nil

local function NexusKamuiEffectStart()
	pcall(function()
		local camera = Workspace.CurrentCamera
		if camera then
			NexusKamuiBaseFov = camera.FieldOfView
			TweenService:Create(camera, TweenInfo.new(0.12, Enum.EasingStyle.Quad), {FieldOfView = math.clamp((NexusKamuiBaseFov or 70) - 10, 45, 100)}):Play()
		end

		NexusKamuiFxGui = Instance.new("ScreenGui")
		NexusKamuiFxGui.Name = "NexusKamuiFX"
		NexusKamuiFxGui.IgnoreGuiInset = true
		NexusKamuiFxGui.ResetOnSpawn = false
		NexusKamuiFxGui.DisplayOrder = 1000

		local root = Instance.new("Frame")
		root.Size = UDim2.fromScale(1, 1)
		root.BackgroundTransparency = 1
		root.Parent = NexusKamuiFxGui

		local shade = Instance.new("Frame")
		shade.Size = UDim2.fromScale(1, 1)
		shade.BackgroundColor3 = OBITO_PHASE_COLOR
		shade.BackgroundTransparency = 0.90
		...
		center.Size = UDim2.fromOffset(38, 38)
		...
		for i = 1, 4 do
			...
			ring.Size = UDim2.fromOffset(55 + i * 42, 55 + i * 42)
			ring.BackgroundTransparency = 1
			ring.Parent = center

			local stroke = Instance.new("UIStroke")
			stroke.Color = i % 2 == 0 and Color3.fromRGB(110, 65, 255) or Color3.fromRGB(210, 130, 255)
			stroke.Thickness = 2
			stroke.Transparency = 0.18 + i * 0.08
			stroke.Parent = ring
		...
		label.Text = "KAMUI"
		label.TextColor3 = Color3.fromRGB(235, 215, 255)
		label.TextSize = 20
		...
		local parent = nil
		if Player then parent = Player:FindFirstChildOfClass("PlayerGui") end
		if not parent then
			pcall(function() if gethui then parent = gethui() end end)
		end
		if parent then NexusKamuiFxGui.Parent = parent end

		NexusKamuiBlur = Instance.new("BlurEffect")
		NexusKamuiBlur.Name = "NexusKamuiBlur"
		NexusKamuiBlur.Size = 0
		NexusKamuiBlur.Parent = Lighting
		TweenService:Create(NexusKamuiBlur, TweenInfo.new(0.10), {Size = 10}):Play()

		NexusKamuiColor = Instance.new("ColorCorrectionEffect")
		NexusKamuiColor.Name = "NexusKamuiColor"
		NexusKamuiColor.TintColor = Color3.fromRGB(185, 165, 255)
		NexusKamuiColor.Contrast = 0.18
		NexusKamuiColor.Saturation = -0.15
		NexusKamuiColor.Parent = Lighting

		NexusKamuiFxConnection = RunService.RenderStepped:Connect(function()
			local t = os.clock()
			for i, ring in ipairs(rings) do
				ring.Rotation = (t * (i % 2 == 0 and -90 or 65) + i * 24) % 360
			end
			center.Rotation = (t * 20) % 360
			shade.BackgroundTransparency = 0.88 + (math.sin(t * 12) + 1) * 0.025
		end)
	end)
end

local function NexusKamuiEffectStop()
	pcall(function()
		if NexusKamuiFxConnection then NexusKamuiFxConnection:Disconnect() end
		NexusKamuiFxConnection = nil

		if NexusKamuiBlur then
			NexusKamuiBlur:Destroy()
			NexusKamuiBlur = nil
		end
		if NexusKamuiColor then
			NexusKamuiColor:Destroy()
			NexusKamuiColor = nil
		end

		local camera = Workspace.CurrentCamera
		if camera and NexusKamuiBaseFov then
			TweenService:Create(camera, TweenInfo.new(0.18, Enum.EasingStyle.Quad), {FieldOfView = NexusKamuiBaseFov}):Play()
		end
		NexusKamuiBaseFov = nil

		if NexusKamuiFxGui then NexusKamuiFxGui:Destroy() end
		NexusKamuiFxGui = nil
	end)
end
...
local function ObitoPhaseOn()
    ...
    NexusKamuiEffectStart()

    -- BUGFIX: qualquer dano recebido durante o Kamui volta para o HP
    -- exato que existia no instante em que a fase começou.
    table.insert(ObitoPhaseConnections,h.HealthChanged:Connect(function(newHealth)
        if not ObitoPhaseActive or ObitoPhaseCharacter ~= c then return end
        local saved=ObitoPhaseHealthBefore
        if saved and newHealth < saved then
            ObitoPhaseDamageTaken = math.max(ObitoPhaseDamageTaken, saved - newHealth)
            local target=math.min(saved,h.MaxHealth)
            pcall(function() h.Health=target end)
            task.defer(function()
                if ObitoPhaseActive and ObitoPhaseCharacter == c and h.Parent and h.Health > 0 and h.Health < target then
                    pcall(function() h.Health=target end)
                end
            end)
            ObitoPhaseHealthLast=target
        else
...
function RefreshObitoStatus()
	pcall(function()
		if not ObitoStatusLabel then return end
		local now=os.clock()
		if ObitoPhaseActive then
			local left=math.max(0,ObitoPhaseUntil-now)
			ObitoStatusLabel.Text=string.format("KAMUI: ACTIVE  %.2fs  |  DAMAGE: %.0f", left, ObitoPhaseDamageTaken)
			ObitoStatusLabel.TextColor3=GREEN
		elseif now < ObitoPhaseCooldownUntil then
			local left=math.max(0,ObitoPhaseCooldownUntil-now)
			ObitoStatusLabel.Text=string.format("KAMUI: COOLDOWN  %.2fs", left)
			ObitoStatusLabel.TextColor3=YELLOW
		else
			ObitoStatusLabel.Text=string.format("KAMUI: READY  |  LAST: %s", ObitoPhaseLastReason ~= "" and ObitoPhaseLastReason or "—")
			ObitoStatusLabel.TextColor3=PURPLE
		end
	end)
end
...
_G.NexusFPSBoosterV20_Cleanup = function()
	...
	pcall(function()
		if FPSRenderConnection then FPSRenderConnection:Disconnect() end
		FPSRenderConnection=nil
	end)
	pcall(function()
		if ObitoPhaseHeartbeatConnection then ObitoPhaseHeartbeatConnection:Disconnect() end
		ObitoPhaseHeartbeatConnection=nil
		ObitoDisconnectAll()
	end)
...
QuickButton.Name = "NexusQuickButton"
QuickButton.Size = UDim2.fromOffset(34, 34)
QuickButton.Position = UDim2.new(1, -122, 0, 8)
QuickButton.BackgroundColor3 = PANEL2
QuickButton.Text = "⌂"
QuickButton.TextColor3 = WHITE
QuickButton.TextSize = 17
QuickButton.Font = Enum.Font.GothamBold
QuickButton.AutoButtonColor = false
QuickButton.Parent = Top

QuickCorner = Instance.new("UICorner")
...
-- Botão de minimizar ao lado da aba EMOTES.
EmoteMinimizeButton = Instance.new("TextButton")
EmoteMinimizeButton.Name = "EmoteMinimizeButton"
EmoteMinimizeButton.Size = UDim2.fromOffset(32, 32)
EmoteMinimizeButton.Position = UDim2.fromOffset(496, 55)
EmoteMinimizeButton.BackgroundColor3 = PANEL2
EmoteMinimizeButton.Text = "—"
EmoteMinimizeButton.TextColor3 = WHITE
EmoteMinimizeButton.TextSize = 16
EmoteMinimizeButton.Font = Enum.Font.GothamBold
EmoteMinimizeButton.AutoButtonColor = false
EmoteMinimizeButton.Parent = Main

EmoteMinimizeCorner = Instance.new("UICorner")
EmoteMinimizeCorner.CornerRadius = UDim.new(0, 8)
EmoteMinimizeCorner.Parent = EmoteMinimizeButton
...
ObitoStatusLabel=Instance.new("TextLabel")
ObitoStatusLabel.Size=UDim2.new(1,0,0,22)
ObitoStatusLabel.Position=UDim2.fromOffset(0,348)
ObitoStatusLabel.BackgroundTransparency=1
ObitoStatusLabel.Text="KAMUI: READY"
ObitoStatusLabel.TextColor3=PURPLE
ObitoStatusLabel.TextSize=10
ObitoStatusLabel.Font=Enum.Font.GothamBold
ObitoStatusLabel.TextXAlignment=Enum.TextXAlignment.Left
ObitoStatusLabel.Parent=BoostPage
RefreshObitoStatus()
...
NexusBootStatus("GUI criada • V20.3")
task.delay(2, function()
...
RefreshAimControls()
RefreshFlyControls()
if NexusQuickFrame then
	NexusQuickOpen = true
	NexusQuickFrame.Visible = true
	BoostPage.Visible = false
	InfoPage.Visible = false
	PlayerPage.Visible = false
	EmotePage.Visible = false
end
ApplyNexusTheme(ThemeName)
ApplyUITransparency(UITransparency)
PLAYER_AURA_ENABLED=PlayerAuraEnabled
RefreshObitoStatus()
...
function SelectTab(tab)
	if NexusSettingsFrame then
		NexusSettingsOpen = false
		NexusSettingsFrame.Visible = false
	end
	NexusQuickOpen = false
	if NexusQuickFrame then NexusQuickFrame.Visible = false end
...
--==================================================
-- MODO SIMPLES / PAINEL RÁPIDO
--==================================================

NexusQuickOpen = true
NexusQuickFrame = Instance.new("Frame")
NexusQuickFrame.Name = "NexusQuickFrame"
NexusQuickFrame.Size = BoostPage.Size
NexusQuickFrame.Position = BoostPage.Position
NexusQuickFrame.BackgroundTransparency = 1
NexusQuickFrame.Visible = true
NexusQuickFrame.Parent = Main

NexusQuickTitle = Instance.new("TextLabel")
NexusQuickTitle.Size = UDim2.new(1, 0, 0, 28)
NexusQuickTitle.BackgroundTransparency = 1
NexusQuickTitle.Text = "N E X U S  //  MENU RÁPIDO"
NexusQuickTitle.TextColor3 = WHITE
NexusQuickTitle.TextSize = 15
NexusQuickTitle.Font = Enum.Font.GothamBold
NexusQuickTitle.TextXAlignment = Enum.TextXAlignment.Left
NexusQuickTitle.Parent = NexusQuickFrame

NexusQuickStatus = Instance.new("TextLabel")
NexusQuickStatus.Size = UDim2.new(1, 0, 0, 54)
NexusQuickStatus.Position = UDim2.fromOffset(0, 34)
NexusQuickStatus.BackgroundColor3 = PANEL2
NexusQuickStatus.BackgroundTransparency = 0.15
NexusQuickStatus.TextColor3 = WHITE
NexusQuickStatus.TextSize = 11
NexusQuickStatus.Font = Enum.Font.Code
NexusQuickStatus.TextXAlignment = Enum.TextXAlignment.Left
NexusQuickStatus.TextYAlignment = Enum.TextYAlignment.Center
NexusQuickStatus.Text = "FPS  --    PING  --    RAM  --\nBOOST: --    KAMUI: --    AURA: --"
NexusQuickStatus.Parent = NexusQuickFrame

local qCorner = Instance.new("UICorner")
qCorner.CornerRadius = UDim.new(0, 10)
qCorner.Parent = NexusQuickStatus

local function NexusQuickButton(text, y, callback)
...
local QuickBoostButton = NexusQuickButton("⚡  BOOST FPS", 100, function()
	BoosterEnabled = not BoosterEnabled
	UpdateToggle()
end)

local QuickCureButton = NexusQuickButton("❤️  CURA", 292, function()
    local ok, status = NexusCurePlayer()
    if NexusQuickStatus then
        NexusQuickStatus.Text = "CURA: " .. tostring(status)
    end
end)

local QuickKamuiButton = NexusQuickButton("🌀  KAMUI", 148, function()
	if not OBITO_PHASE_ENABLED then
	...
	if ObitoPhaseActive then
		ObitoPhaseOff("quick_button")
	else
		ObitoPhaseOn()
	end
end)

local QuickAuraButton = NexusQuickButton("👁  AURA DOS JOGADORES", 196, function()
	PlayerAuraEnabled = not PlayerAuraEnabled
	PLAYER_AURA_ENABLED = PlayerAuraEnabled
	pcall(PlayerAuraRefreshAll)
	PersistConfig()
end)

local QuickConfigButton = NexusQuickButton("⚙  CONFIGURAÇÕES AVANÇADAS", 244, function()
	NexusQuickOpen = false
	NexusQuickFrame.Visible = false
	NexusSettingsOpen = true
	NexusSettingsFrame.Visible = true
	NexusUpdateDiagnostics()
end)

local QuickInfoButton = NexusQuickButton("📊  INFO / GRÁFICO", 292, function()
	SelectTab("INFO")
end)

local QuickEmoteButton = NexusQuickButton("🎭  EMOTES", 388, function()
	SelectTab("EMOTES")
end)

local function NexusRefreshQuick()
	if not NexusQuickStatus then return end
	local ram = "N/A"
	pcall(function() ram = string.format("%.0f MB", Stats:GetTotalMemoryUsageMb()) end)
	NexusQuickStatus.Text =
		"FPS  " .. tostring(FPS or 0) .. "    PING  " .. tostring(NexusCurrentPing or 0) .. " ms    RAM  " .. ram .. "\n" ..
		"BOOST: " .. (BoosterEnabled and CurrentMode or "OFF") ..
		"    KAMUI: " .. (ObitoPhaseActive and "ATIVO" or "PRONTO") ..
		"    AURA: " .. (PlayerAuraEnabled and "ON" or "OFF")
end

task.spawn(function()
	while Gui.Parent do
		if NexusQuickOpen then pcall(NexusRefreshQuick) end
		task.wait(0.5)
	end
end)
...
local function NexusToggleMinimize()
	...
	EmoteMinimizeButton.Visible = not Minimized
	QuickButton.Visible = not Minimized
	...
	if NexusSettingsFrame then NexusSettingsFrame.Visible = not Minimized and NexusSettingsOpen end
	if NexusQuickFrame then NexusQuickFrame.Visible = not Minimized and NexusQuickOpen end

	MinimizeButton.Text = Minimized and "▢" or "—"
	EmoteMinimizeButton.Text = Minimized and "▢" or "—"
end

MinimizeButton.MouseButton1Click:Connect(NexusToggleMinimize)
EmoteMinimizeButton.MouseButton1Click:Connect(NexusToggleMinimize)
...
--==================================================
-- V20.3 SIMPLE SETTINGS / PROFILES / DIAGNOSTICS
-- Interface reorganizada para uso rápido no celular.
--==================================================
NexusProfiles = NexusProfiles or {}
NexusSettingsOpen = false
NexusSettingsFrame = nil
NexusDiagnosticsLabel = nil

local function NexusSnapshotConfig()
	return {
		CurrentMode = CurrentMode,
		AntiLagEnabled = AntiLagEnabled,
		EssentialOnly = EssentialOnly,
...
local function NexusApplySnapshot(c)
	if type(c) ~= "table" then return end
	CurrentMode = c.CurrentMode or CurrentMode
	AntiLagEnabled = c.AntiLagEnabled == true
	EssentialOnly = c.EssentialOnly == true
	SpeedEnabled = c.SpeedEnabled == true
	SpeedValue = math.clamp(tonumber(c.SpeedValue) or SpeedValue, 20, 400)
	FlyEnabled = c.FlyEnabled == true
	FlySpeed = math.clamp(tonumber(c.FlySpeed) or FlySpeed, 20, 150)
	ThemeName = NexusThemes[c.ThemeName] and c.ThemeName or ThemeName
	UITransparency = math.clamp(tonumber(c.UITransparency) or UITransparency, 0, 0.75)
	PlayerAuraMaxDistance = math.clamp(tonumber(c.PlayerAuraMaxDistance) or PlayerAuraMaxDistance, 25, 1000)
	PlayerAuraEnabled = c.PlayerAuraEnabled ~= false
	PlayerAuraColorMode = c.PlayerAuraColorMode or PlayerAuraColorMode
	OBITO_PHASE_DURATION = math.clamp(tonumber(c.ObitoDuration) or OBITO_PHASE_DURATION, 0.05, 2)
	OBITO_PHASE_COOLDOWN = math.clamp(tonumber(c.ObitoCooldown) or OBITO_PHASE_COOLDOWN, 0, 10)
	OBITO_PHASE_ENABLED = c.ObitoEnabled ~= false
	ApplyNexusTheme(ThemeName)
	ApplyUITransparency(UITransparency)
	ApplyAntiLag(AntiLagEnabled)
	ApplyEssentialOnly(EssentialOnly)
...
local function NexusSaveProfile(name)
	name = tostring(name or ""):upper()
	if name == "" then return end
	NexusProfiles[name] = NexusSnapshotConfig()
	SaveConfig({
		CurrentMode = CurrentMode,
		AntiLagEnabled = AntiLagEnabled,
		EssentialOnly = EssentialOnly,
		...
		ObitoCooldown = OBITO_PHASE_COOLDOWN,
		ObitoEnabled = OBITO_PHASE_ENABLED,
		Profiles = NexusProfiles,
	})
end

local function NexusLoadProfile(name)
	local c = NexusProfiles[tostring(name or ""):upper()]
	if c then NexusApplySnapshot(c) end
end

if type(SavedConfig) == "table" and type(SavedConfig.Profiles) == "table" then
	NexusProfiles = SavedConfig.Profiles
end

local function NexusUpdateDiagnostics()
	if not NexusDiagnosticsLabel then return end
	local character = Player and Player.Character
	local humanoid = character and character:FindFirstChildOfClass("Humanoid")
	local root = character and character:FindFirstChild("HumanoidRootPart")
	local auraCount = 0
	for _ in pairs(PlayerAuraObjects or {}) do auraCount += 1 end
	local mem = "N/A"
	pcall(function() mem = string.format("%.1f MB", Stats:GetTotalMemoryUsageMb()) end)

	NexusDiagnosticsLabel.Text =
		"STATUS DO NEXUS\n\n" ..
		"Personagem     " .. (character and "✓ OK" or "✗ AUSENTE") .. "\n" ..
		"Humanoid       " .. (humanoid and "✓ OK" or "✗ AUSENTE") .. "\n" ..
		"RootPart       " .. (root and "✓ OK" or "✗ AUSENTE") .. "\n" ..
		"Kamui          " .. (ObitoPhaseActive and "● ATIVO" or "○ PRONTO") .. "\n" ..
		"Auras ativas   " .. tostring(auraCount) .. "\n" ..
		"FPS            " .. tostring(FPS or 0) .. "\n" ..
		"RAM            " .. tostring(mem) .. "\n\n" ..
		"Tema: " .. tostring(ThemeName) .. "\n" ..
		"Modo: " .. tostring(CurrentMode)
end

SettingsButton = Instance.new("TextButton")
SettingsButton.Name = "NexusSettingsButton"
SettingsButton.Size = UDim2.fromOffset(34, 34)
SettingsButton.Position = UDim2.new(1, -84, 0, 8)
SettingsButton.BackgroundColor3 = PANEL2
SettingsButton.Text = "⚙"
...
NexusSettingsFrame = Instance.new("ScrollingFrame")
NexusSettingsFrame.Name = "NexusSettings"
NexusSettingsFrame.Size = UDim2.new(1, -24, 1, -92)
NexusSettingsFrame.Position = UDim2.fromOffset(12, 88)
NexusSettingsFrame.BackgroundColor3 = Color3.fromRGB(12, 8, 18)
NexusSettingsFrame.BackgroundTransparency = 0.04
NexusSettingsFrame.BorderSizePixel = 0
NexusSettingsFrame.ScrollBarThickness = 4
NexusSettingsFrame.CanvasSize = UDim2.fromOffset(0, 0)
NexusSettingsFrame.Visible = false
NexusSettingsFrame.ZIndex = 100
NexusSettingsFrame.Parent = Main

do
	local c = Instance.new("UICorner")
	c.CornerRadius = UDim.new(0, 12)
	c.Parent = NexusSettingsFrame
	local st = Instance.new("UIStroke")
	st.Color = PURPLE
	st.Thickness = 1.5
	st.Parent = NexusSettingsFrame
end
...
settingsLayout.Parent = NexusSettingsFrame
...
settingsPadding.Parent = NexusSettingsFrame

local function NexusSettingsSection(text, order)
	local label = Instance.new("TextLabel")
	...
	label.ZIndex = 101
	label.Parent = NexusSettingsFrame
end

local function NexusSettingsRow(title, description, order, callback, getValue)
	...
	titleLabel.Size = UDim2.new(1, -125, 0, 24)
	...
	descLabel.TextColor3 = Color3.fromRGB(170, 160, 180)
	...
	value.Size = UDim2.fromOffset(100, 36)
	value.Position = UDim2.new(1, -110, 0.5, -18)
	value.BackgroundColor3 = Color3.fromRGB(18, 12, 26)
	...
	local function refreshValue()
		...
		local ok, result = pcall(getValue)
		if ok and result ~= nil then value.Text = tostring(result) end
	end
...
NexusSettingsSection("🎨 APARÊNCIA", 1)
NexusSettingsRow("Tema", "Muda as cores e o estilo do menu.", 2, function()
	local order = {"NEXUS", "AKATSUKI", "OBITO", "CYBER"}
	local index = table.find(order, ThemeName) or 1
	ThemeName = order[index % #order + 1]
	ApplyNexusTheme(ThemeName)
	PersistConfig()
	return ThemeName
end, function() return ThemeName end)

NexusSettingsRow("Transparência", "Deixa o painel mais transparente.", 3, function()
	UITransparency += 0.10
	if UITransparency > 0.70 then UITransparency = 0 end
	ApplyUITransparency(UITransparency)
	PersistConfig()
	return math.floor(UITransparency * 100) .. "%"
end, function() return math.floor(UITransparency * 100) .. "%" end)

NexusSettingsSection("🌀 KAMUI", 4)
NexusSettingsRow("Kamui", "Ativa ou desativa a habilidade de fase.", 5, function()
	OBITO_PHASE_ENABLED = not OBITO_PHASE_ENABLED
	if not OBITO_PHASE_ENABLED then ObitoPhaseOff("settings_disabled") end
	PersistConfig()
	return OBITO_PHASE_ENABLED and "ATIVO" or "DESLIGADO"
end, function() return OBITO_PHASE_ENABLED and "ATIVO" or "DESLIGADO" end)

NexusSettingsRow("Duração", "Quanto tempo o Kamui fica ativo.", 6, function()
	OBITO_PHASE_DURATION += 0.05
	if OBITO_PHASE_DURATION > 0.50 then OBITO_PHASE_DURATION = 0.05 end
	PersistConfig()
	return string.format("%.2fs", OBITO_PHASE_DURATION)
end, function() return string.format("%.2fs", OBITO_PHASE_DURATION) end)

NexusSettingsSection("👁️ AURA", 8)
NexusSettingsRow("Aura dos jogadores", "Mostra um contorno nos jogadores próximos.", 9, function()
	PlayerAuraEnabled = not PlayerAuraEnabled
	PLAYER_AURA_ENABLED = PlayerAuraEnabled
	pcall(PlayerAuraRefreshAll)
	PersistConfig()
	return PlayerAuraEnabled and "ATIVA" or "DESLIGADA"
end, function() return PlayerAuraEnabled and "ATIVA" or "DESLIGADA" end)

NexusSettingsRow("Distância", "Até onde a aura dos jogadores aparece.", 10, function()
	PlayerAuraMaxDistance += 50
	if PlayerAuraMaxDistance > 500 then PlayerAuraMaxDistance = 50 end
	pcall(PlayerAuraRefreshAll)
	PersistConfig()
	return PlayerAuraMaxDistance .. " studs"
end, function() return PlayerAuraMaxDistance .. " studs" end)

NexusSettingsSection("⚡ DESEMPENHO", 11)
NexusSettingsRow("Auto Performance", "Reduz o modo gráfico se o FPS ficar muito baixo.", 12, function()
	NexusAutoPerformance = not NexusAutoPerformance
	PersistConfig()
	return NexusAutoPerformance and "ATIVO" or "DESLIGADO"
end, function() return NexusAutoPerformance and "ATIVO" or "DESLIGADO" end)

NexusSettingsRow("Limite de FPS", "Limita o FPS quando o ambiente oferece suporte.", 13, function()
	FPS_CAP = (FPS_CAP or 0) + 30
	if FPS_CAP > 240 then FPS_CAP = 0 end
	NexusApplyFPSCap()
	PersistConfig()
	return FPS_CAP == 0 and "SEM LIMITE" or (FPS_CAP .. " FPS")
end, function() return FPS_CAP == 0 and "SEM LIMITE" or (FPS_CAP .. " FPS") end)

NexusSettingsSection("💾 PERFIL", 14)
NexusSettingsRow("Salvar perfil", "Guarda suas configurações atuais.", 15, function()
	NexusSaveProfile("NEXUS")
	return "SALVO"
end, function() return "SALVO" end)

NexusSettingsRow("Carregar perfil", "Restaura o perfil Nexus salvo.", 16, function()
	NexusLoadProfile("NEXUS")
	return "CARREGADO"
end, function() return "CARREGADO" end)

NexusSettingsSection("❤️ CURA", 17)
NexusSettingsRow("Cura", "Tenta restaurar o HP do personagem ao máximo.", 18, function()
    local ok, status = NexusCurePlayer()
    return status
end, function() return "CURAR" end)

NexusSettingsSection("🩺 DIAGNÓSTICO", 19)
NexusSettingsRow("Ver diagnóstico", "Mostra o estado do personagem, Kamui, aura, FPS e RAM.", 20, function()
	NexusUpdateDiagnostics()
	return "ATUALIZADO"
end, function() return "ATUALIZAR" end)

NexusDiagnosticsLabel = Instance.new("TextLabel")
NexusDiagnosticsLabel.Size = UDim2.new(1, -20, 0, 180)
NexusDiagnosticsLabel.BackgroundColor3 = Color3.fromRGB(8, 6, 12)
NexusDiagnosticsLabel.BackgroundTransparency = 0.15
NexusDiagnosticsLabel.TextColor3 = WHITE
NexusDiagnosticsLabel.TextSize = 10
NexusDiagnosticsLabel.Font = Enum.Font.Code
NexusDiagnosticsLabel.TextXAlignment = Enum.TextXAlignment.Left
NexusDiagnosticsLabel.TextYAlignment = Enum.TextYAlignment.Top
NexusDiagnosticsLabel.LayoutOrder = 21
NexusDiagnosticsLabel.ZIndex = 101
NexusDiagnosticsLabel.Parent = NexusSettingsFrame

do
	local c = Instance.new("UICorner")
	c.CornerRadius = UDim.new(0, 8)
	c.Parent = NexusDiagnosticsLabel
end

local function NexusRefreshSettingsCanvas()
	NexusSettingsFrame.CanvasSize = UDim2.fromOffset(0, settingsLayout.AbsoluteContentSize.Y + 24)
end
settingsLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(NexusRefreshSettingsCanvas)

local function NexusToggleSettings()
	NexusSettingsOpen = not NexusSettingsOpen
	if NexusSettingsOpen then
		NexusQuickOpen = false
		if NexusQuickFrame then NexusQuickFrame.Visible = false end
	end
	NexusSettingsFrame.Visible = NexusSettingsOpen and not Minimized
	if NexusSettingsOpen then NexusUpdateDiagnostics() end
end
SettingsButton.Activated:Connect(NexusToggleSettings)

QuickButton.Activated:Connect(function()
	NexusSettingsOpen = false
	if NexusSettingsFrame then NexusSettingsFrame.Visible = false end
	NexusQuickOpen = not NexusQuickOpen
	if NexusQuickFrame then NexusQuickFrame.Visible = NexusQuickOpen end
	if NexusQuickOpen then
		BoostPage.Visible = false
		InfoPage.Visible = false
		PlayerPage.Visible = false
		EmotePage.Visible = false
...
FPS_CAP = tonumber(SavedConfig and SavedConfig.FPS_CAP) or 0
NexusAutoPerformance = SavedConfig and SavedConfig.NexusAutoPerformance == true or false
NexusKamuiVisualEnabled = false

local function NexusApplyFPSCap()
	pcall(function()
		if type(setfpscap) == "function" and FPS_CAP > 0 then setfpscap(FPS_CAP) end
	end)
end
NexusApplyFPSCap()

-- V20.3: efeito visual do Kamui removido.
NexusKamuiVisualEnabled = false

local NexusLowFPSStreak = 0
task.spawn(function()
	while Gui and Gui.Parent do
		task.wait(3)
		if NexusAutoPerformance and FPS > 0 then
			if FPS < 25 then NexusLowFPSStreak += 1 else NexusLowFPSStreak = 0 end
			if NexusLowFPSStreak >= 2 and BoosterEnabled and CurrentMode ~= "BATATA" then
				CurrentMode = "BATATA"
				pcall(ApplyBoost)
				NexusLowFPSStreak = 0
			end
		end
		pcall(NexusUpdateDiagnostics)
	end
end)
...
Player.CharacterAdded:Connect(function()
	task.defer(function()
		NexusKamuiEffectStop()
		if ObitoPhaseActive then ObitoPhaseOff("respawn") end
		pcall(NexusUpdateDiagnostics)
	end)
end)
...
pcall(function()
	...
	-- PlayerAuraHeartbeatConnection remains active for the lifetime of the script.
	RefreshObitoStatus()
end)
