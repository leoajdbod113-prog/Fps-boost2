-- NexusFPSBooster V21.1-A
-- Diagnostic build based on the supplied V21.
-- Roblox Studio / own test environment.
-- Fly and Speed intentionally disabled for this diagnostic build.
...
local function __diagSafeText(v)
    local ok, s = pcall(tostring, v)
    return ok and s or "<unprintable error>"
end

local function __diagLog(msg)
    print("[NEXUS] " .. __diagSafeText(msg))
end

local function __diagCreateGui()
    local Players = game:GetService("Players")
    local player = Players.LocalPlayer
    if not player then error("LocalPlayer is nil") end

    local playerGui = player:WaitForChild("PlayerGui", 10)
    if not playerGui then error("PlayerGui was not found within 10 seconds") end

    local old = playerGui:FindFirstChild("NexusV21_1A_Diagnostic")
    if old then old:Destroy() end

    local gui = Instance.new("ScreenGui")
    gui.Name = "NexusV21_1A_Diagnostic"
    ...
    frame.Size = UDim2.fromOffset(390, 250)
    frame.Position = UDim2.new(0, 18, 0.5, -125)
    ...
    status.TextColor3 = Color3.fromRGB(210, 190, 255)
    status.Text = "STATUS: INITIALIZING"
    status.Parent = frame

    local log = Instance.new("TextLabel")
    log.BackgroundTransparency = 1
    log.Size = UDim2.new(1, -24, 1, -78)
    log.Position = UDim2.fromOffset(12, 70)
    log.Font = Enum.Font.Code
    log.TextSize = 13
    log.TextWrapped = false
    log.TextXAlignment = Enum.TextXAlignment.Left
    log.TextYAlignment = Enum.TextYAlignment.Top
    log.TextColor3 = Color3.fromRGB(220, 220, 230)
    log.Text = ""
    log.Parent = frame
...
local __diagUi
do
    local ok, result = xpcall(__diagCreateGui, debug.traceback)
    if not ok then
        warn("[NEXUS] Bootstrap GUI ERROR: " .. __diagSafeText(result))
        return
    end
    __diagUi = result
end

local function __diagStep(number, name, fn)
    __NEXUS_DIAG.step = number
    __diagUi.addLine(("[%d] %s"):format(number, name))
    __diagLog(("%d. %s"):format(number, name))

    local ok, result = xpcall(fn, debug.traceback)
    if not ok then
        __NEXUS_DIAG.failed = true
        __NEXUS_DIAG.status = "ERROR"
        __diagUi.setStatus("ERROR")
        __diagUi.addLine("!")
        __diagUi.addLine("FAILED: " .. name)
        __diagUi.addLine(__diagSafeText(result):sub(1, 180))
        __diagLog("ERROR in " .. name .. ": " .. __diagSafeText(result))
        return false
    end
...
__diagStep(1, "Script iniciado", function() end)

local Players, RunService, player, playerGui, camera, character, humanoid

if not __diagStep(2, "Services OK", function()
...
if not __diagStep(3, "LocalPlayer OK", function()
    player = Players.LocalPlayer
    assert(player, "Players.LocalPlayer is nil")
end) then return end

if not __diagStep(4, "PlayerGui OK", function()
    playerGui = player:WaitForChild("PlayerGui", 10)
    assert(playerGui, "PlayerGui unavailable")
end) then return end

__diagStep(5, "GUI criada", function()
    assert(__diagUi and __diagUi.gui and __diagUi.gui.Parent, "Diagnostic GUI was not parented")
end)

if not __diagStep(6, "Camera OK", function()
    camera = workspace.CurrentCamera
    assert(camera, "CurrentCamera is nil")
end) then return end

if not __diagStep(7, "Character OK", function()
    character = player.Character or player.CharacterAdded:Wait()
    assert(character, "Character unavailable")
end) then return end

if not __diagStep(8, "Humanoid OK", function()
    humanoid = character:FindFirstChildOfClass("Humanoid")
    assert(humanoid, "Humanoid not found")
end) then return end

if not __diagStep(9, "RenderStepped OK", function()
    assert(RunService.RenderStepped, "RenderStepped unavailable")
end) then return end

__NEXUS_DIAG.status = "READY"
__diagUi.setStatus("READY")
__diagUi.addLine("[NEXUS] READY")
__diagLog("READY")

-- Signals to the original V21 that the diagnostic bootstrap completed.
-- The original V21 is then executed below in the same LocalScript.
_G.NEXUS_V21_1A_BOOTSTRAP_OK = true
_G.NEXUS_V21_1A_DISABLE_SPEED = true
_G.NEXUS_V21_1A_DISABLE_FLY = true


-- ============================================================
-- ORIGINAL NEXUS V21 CODE (preserved as supplied)
-- ============================================================
...
-- V21.1-A diagnostic wrapper reached the end of the supplied V21 source.
pcall(function()
    local pg = game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui")
    local d = pg and pg:FindFirstChild("NexusV21_1A_Diagnostic")
    if d then
        local status = d:FindFirstChildWhichIsA("Frame") and d:FindFirstChildWhichIsA("Frame"):FindFirstChild("TextLabel")
        -- Keep the existing diagnostic status untouched if the original V21
        -- creates its own UI. The important result is logged to the console.
    end
    print("[NEXUS] V21.1-A reached end of source")
end)
