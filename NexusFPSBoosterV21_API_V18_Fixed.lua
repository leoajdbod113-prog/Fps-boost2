...
API.Lighting = game:GetService("Lighting")
API.Workspace = game:GetService("Workspace")
API.UserInputService = game:GetService("UserInputService")
API.Stats = game:GetService("Stats")
API.StarterGui = game:GetService("StarterGui")
API.SoundService = game:GetService("SoundService")
API.HttpService = game:GetService("HttpService")

API.Player = API.Players.LocalPlayer
API.Camera = API.Workspace.CurrentCamera
...
local StarterGui = API.StarterGui
local SoundService = API.SoundService
local HttpService = API.HttpService
local Player = API.Player
local Camera = API.Camera

print("[NEXUS] API V18 bridge OK")
print("[NEXUS] LocalPlayer:", Player and Player.Name or "nil")
...
local AimBindOK, AimBindErr = pcall(function()
	RunService:BindToRenderStep('NexusAimLock', LOCKON_CAMERA_PRIORITY, function(dt)
		UpdateAimLock(dt)
	end)
end)
if AimBindOK then
	print('[NEXUS] AimLock RenderStep OK')
else
	warn('[NEXUS] AimLock RenderStep indisponível:', AimBindErr)
end
...
print("[NEXUS] Chegou na GUI BASE")
local Gui = Instance.new("ScreenGui")
Gui.Name = "NexusFPSBoosterV9"
Gui.ResetOnSpawn = false
Gui.IgnoreGuiInset = true
Gui.Parent = Player:WaitForChild("PlayerGui")
print("[NEXUS] GUI criada e parentada no PlayerGui")
